...
public:
    ...
    friend  void order(Furniture** fp1, Furniture** fp2);
    friend  void bsort(Furniture** fp, int n);
};
...
**/
void bsort(Furniture** fp, int n) //sort pointers to persons
{
    void order(Furniture**, Furniture**); //prototype
    int j, k; //indexes to array
    for(j=0; j<n-1; j++) //outer loop
        for(k=j+1; k<n; k++) //inner loop starts at outer
            order(fp+j, fp+k); //order the pointer contents
}
//--------------------------------------------------------------
void order(Furniture** fp1, Furniture** fp2) //orders two pointers
{ //if 1st larger than 2nd,
    if( (*fp1)->discount < (*fp2)->discount )
    {
        Furniture* tempptr = *fp1; //swap the pointers
        *fp1 = *fp2;
        *fp2 = tempptr;
    }
}
